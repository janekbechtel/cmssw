...
template <typename T1, typename T2>
void  CollectionMerger<T1,T2>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
...
template <>
void  CollectionMerger<edmNew::DetSetVector<SiPixelCluster>, SiPixelCluster>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
...
template <>
void  CollectionMerger<edmNew::DetSetVector<SiStripCluster>, SiStripCluster>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
...
template <>
void  CollectionMerger<edm::SortedCollection<EcalRecHit>,  EcalRecHit>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
...
template <>
void  CollectionMerger<edm::SortedCollection<HBHERecHit>, HBHERecHit>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
  fill_output_obj_calo(output,inputCollections);
}

template <>
void  CollectionMerger<edm::SortedCollection<HFRecHit>, HFRecHit>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
  fill_output_obj_calo(output,inputCollections);
}

template <>
void  CollectionMerger<edm::SortedCollection<HORecHit>, HORecHit>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
  fill_output_obj_calo(output,inputCollections);
}

template <>
void  CollectionMerger<edm::SortedCollection<CastorRecHit>, CastorRecHit>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
  fill_output_obj_calo(output,inputCollections);
}
...
template <>
void  CollectionMerger<edm::SortedCollection<ZDCRecHit>, ZDCRecHit>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
  fill_output_obj_calo(output,inputCollections);
}
...
template <>
void  CollectionMerger<edm::RangeMap<DTLayerId, edm::OwnVector<DTRecHit1DPair> >, DTRecHit1DPair >::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
  fill_output_obj_muonchamber(output,inputCollections);
}

template <>
void  CollectionMerger<edm::RangeMap<CSCDetId, edm::OwnVector<CSCRecHit2D> >, CSCRecHit2D >::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
  fill_output_obj_muonchamber(output,inputCollections);
}
...
template <>
void  CollectionMerger<edm::RangeMap<RPCDetId, edm::OwnVector<RPCRecHit> >, RPCRecHit>::fill_output_obj(edm::Event& iEvent, std::unique_ptr<MergeCollection > & output, std::vector<edm::Handle<MergeCollection> > &inputCollections)
{
  fill_output_obj_muonchamber(output,inputCollections);
}
